...
import { runOngoingInactivityCheck, runDraftExpirationCheck } from './jobs/courseLifecycleJobs.js';
...
const startServer = async () => {
  ...
  cron.schedule('0 3 * * *', async () => {
    try {
      ...
      logger.error('Ongoing inactivity check failed', { error: error.message, stack: error.stack });
    }
  });
  cron.schedule('15 3 * * *', async () => {
    try {
      ...
      logger.error('Draft expiration check failed', { error: error.message, stack: error.stack });
    }
  });
};
