...
export default function CurriculumBuilderTab({ courses = [], lessonsByCourse = {}, onOpenAddLesson, onOpenEditLesson, onEditCourse, onDeleteCourse, onAction }) {
  ...
  // Sync local lessons when props change
  React.useEffect(() => {
    if (selectedCourseId && lessonsByCourse[selectedCourseId]) {
      setLocalLessons(lessonsByCourse[selectedCourseId]);
    } else {
      setLocalLessons([]);
    }
  }, [selectedCourseId, lessonsByCourse]);
  ...
  const handleReorder = async (index, direction) => {
    ...
    try {
      await api.put(`/courses/${selectedCourseId}/lessons-reorder`, { lessonIds });
      if (onAction) onAction();
    } catch (err) {
      console.error(err);
      notyf.error('Failed to reorder lessons');
      // Revert on failure
      setLocalLessons(lessonsByCourse[selectedCourseId] || []);
    }
  };
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {localLessons.length === 0 ? (
                  ...
                  localLessons.map((lesson, index) => (
                    <div key={lesson._id || index} className="glass-card" style={{ padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                          <button
                            ...
                            style={{
                              ...
                              color: index === 0 ? 'var(--border)' : 'var(--c-sub)'
                            }}
                            title="Move up"
                          >
                          ...
                          <button
                            ...
                            style={{
                              cursor: index === localLessons.length - 1 ? 'default' : 'pointer',
                              color: index === localLessons.length - 1 ? 'var(--border)' : 'var(--c-sub)'
                            }}
                            title="Move down"
                          >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        {(() => {
                          ...
                          return (
                            <div style={{ position: 'relative', display: 'flex', background: 'var(--bg-main)', padding: '4px', borderRadius: '50px', boxShadow: 'var(--inner-shadow)', width: '180px', height: '34px' }}>
                              {/* Sliding pill */}
                              <div style={{
                                position: 'absolute',
                                top: '4px',
                              ...
                              <button
                                onClick={() => {
                                  if (isPublished) handleTogglePublish(selectedCourseId, lesson);
                                }}
                                style={{
                                  flex: 1,
                                  position: 'relative',
                                  ...
                                  border: 'none',
                                  background: 'transparent',
                                  color: !isPublished ? 'var(--text-h)' : 'var(--c-sub)',
                                  cursor: 'pointer',
