...
const AdminStatisticsTab = ({ stats, revenueAnalytics, revenueAnalyticsLoading }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }} className="animate-entrance">
      <div>
        <h2 style={{ fontSize: "1.8rem", margin: "0 0 4px 0", color: "var(--text-h)" }}>Statistics</h2>
        <div style={{ fontSize: "0.9rem", color: "var(--c-sub)" }}>Financial summary of platform performance.</div>
      </div>

      <div style={{ marginTop: "16px" }}>
        <div style={{ fontSize: "0.85rem", fontWeight: "700", letterSpacing: "1px", color: "var(--c-sub)", textTransform: "uppercase", marginBottom: "16px" }}>
          FINANCIAL SUMMARY
        </div>
      ...
      <div className="glass-card" style={{ padding: '24px', marginTop: '16px' }}>
        <h3 style={{ margin: "0 0 24px 0", fontSize: "1.1rem", color: "var(--text-h)" }}>Revenue Distribution</h3>
        
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {/* Company Share Bar */}
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.9rem' }}>
              <span style={{ fontWeight: '600', color: 'var(--text-h)' }}>Company Share</span>
              <span style={{ color: 'var(--c-purple)' }}>EGP {companyShare.toLocaleString()} (30%)</span>
            </div>
            <div style={{ width: '100%', height: '8px', background: 'var(--c-input-bg)', borderRadius: '4px', overflow: 'hidden' }}>
              <div style={{ width: '30%', height: '100%', background: 'var(--c-purple)', borderRadius: '4px' }}></div>
            </div>
          </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.9rem' }}>
              <span style={{ fontWeight: '600', color: 'var(--text-h)' }}>Instructor Earnings</span>
              <span style={{ color: 'var(--c-orange)' }}>EGP {instructorEarnings.toLocaleString()} (70%)</span>
            </div>
            <div style={{ width: '100%', height: '8px', background: 'var(--c-input-bg)', borderRadius: '4px', overflow: 'hidden' }}>
              <div style={{ width: '70%', height: '100%', background: 'var(--c-orange)', borderRadius: '4px' }}></div>
            </div>
          </div>
        </div>

        <div style={{ marginTop: '24px', fontSize: '0.8rem', color: 'var(--c-sub)' }}>
          Platform commission is currently configured at <span style={{ color: 'var(--c-light)', fontWeight: '600' }}>30%</span>. This rate is managed by the backend and reflected here automatically.
        </div>
      </div>
    </div>
  );
